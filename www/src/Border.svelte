<script lang="ts">
	...
	export function updateData(data: any, player: number): void {
		...
		if ([0x04, 0x22, 0x23].includes(char))
			mug1Img = `url("mug/cut_${charId}01.png")`;
		else mug1Img = "none";
		moonImg = `url("moon/moon_${moon}.png")`;
	}
</script>

<main class="border" class:flipped>
	{#if !waiting}
		<div
			id="mugheat"
			class="heat"
...
<style lang="scss">
	.border {
		...
		.heat {
			...
			&.heatmode {
				animation: 0.5s linear 0s infinite running heat;
				color: #6a00ff;
			}
		...
		@keyframes heat {
			from {
				filter: brightness(0) /* Top */ drop-shadow(-1px -1px 0px)
					drop-shadow(0px -1px 0px) drop-shadow(1px -1px 0px)
					/* Middle */ drop-shadow(-1px 0px 0px)
					drop-shadow(1px 0px 0px) /* Bottom */
					drop-shadow(-1px 1px 0px) drop-shadow(0px 1px 0px)
					drop-shadow(1px 1px 0px) opacity(100%);
			}

			to {
				filter: brightness(0) /* Top */ drop-shadow(-10px -10px 0px)
					drop-shadow(0px -10px 0px) drop-shadow(10px -10px 0px)
					/* Middle */ drop-shadow(-10px 0px 0px)
					drop-shadow(10px 0px 0px) /* Bottom */
					drop-shadow(-10px 10px 0px) drop-shadow(0px 10px 0px)
					drop-shadow(10px 10px 0px) opacity(0%);
			}
		}
